...
parser.add_argument("--csv-fn", default="pdbbind2019_crystal_core.csv", help="csv file path")
...
def eval():
	...
	with torch.no_grad():
		for bind, batch in enumerate(dataloader):
			...
			if bind == batch_count:
				ytrue_arr[bind*batch_size:len(dataset)] = ytrue
				ypred_arr[bind*batch_size:len(dataset)] = ypred
			else:
				ytrue_arr[bind*batch_size:(bind+1)*batch_size] = ytrue
				ypred_arr[bind*batch_size:(bind+1)*batch_size] = ypred
